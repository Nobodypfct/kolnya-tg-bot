...
const {
  yearOfReviews,
  firstScreenKeyboardText,
  descriptionDocumentToken,
  welcomeMessage,
  errorMessage,
...
bot.on("message", async (msg) => {
  ...
  switch (text) {
    case firstScreenKeyboardText.description:
      ...
      return bot.sendMessage(
        chatId,
        "Задать вопросы или записаться на курс 👉🏼 @hvatiit_maks",
        firstScreenBtnOptions
      );
  ...
  const functionThatReturnsAPromise = async (i) => {
    //a function that returns a promise
    await bot.sendVideo(chatId, i.file_id, {
      caption: i.caption,
      reply_markup: reviewsBtnOptions,
    });
